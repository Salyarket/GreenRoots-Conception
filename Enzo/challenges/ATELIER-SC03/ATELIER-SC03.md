...
Pour cet atelier, l'objetif est d'implémenter un maximum de routes API, ainsi que quelques tests pour chaque route. L'architecture a déjà été mise en place pendant les semaines précédentes. Ainsi, c'est comme en entreprise : on rajoute des fonctionnalités sur une base existante.
...
Pour chaque endpoint (au pire pour chaque demi-journée), penser à créer une **nouvelle branche** puis soumettre et merger celle-ci via l'interface Github. On reste dans l'esprit entreprise ! 

Une fois la **Pull Request** créée et les tests validées par la pipeline (les tests CI passent tous), vous pouvez : 
...
Ordre (suggéré) pour les endspoints : 
- finir les routes des `/levels`
- routes des `/tags`
...
- Rajouter du rate limiting sur la route `register`, `refresh`, et `login`
